...
void bilinearInterpolation( const CPixel* __restrict__ oldPicture, CPixel* __restrict__ newPicture, const SSize& oldPicSize, const SSize& newPicSize )
{
    ...
    for( idx = 0; idx < picTotalSize + cacheAlignment - 1; idx += cacheAlignment )
    {
        for( int newPicIdx = idx; newPicIdx < (picTotalSize > idx+cacheAlignment ? idx+cacheAlignment : picTotalSize); ++newPicIdx )
        {
            ...
            if( x1 == x2 && y1 == y2 )
            {
                newPicture[newPicIdx] = oldPicture[int(y) * oldPicSize._width + int(x)];
            }
            else if( x1 == x2 )
            {
                ...
                newPicture[newPicIdx] = R1 * (y2 - y) + R2 * (y - y1);
            }
            else if( y1 == y2 )
            {
                ...
                newPicture[newPicIdx] = R1 * (x2 - x) + R2 * (x - x1);
            }
            else
            {
                ...
                newPicture[newPicIdx] = float(y2 - y) / float(y2 - y1) * R1 + float(y - y1) / float(y2 - y1) * R2;
            }
        }
    }
}
