...
__device__ float lanczosKernel( const float x )
{
    if( x == 0 ) return 1;
    else if( -1*a < x && x < a )
    {
...
// Lanczos Resampling H function -- resampling horizontally
__global__ void lanczosResamplingH( const CPixel* oldPicture, CPixel* interPicture, const SSize* oldPicSize, const SSize* newPicSize )
{
    const float widthRatio  = float(oldPicSize->_width)  / float(newPicSize->_width);
    const int idx           = threadIdx.x + (blockIdx.x * blockDim.x);
    if( idx >= int(newPicSize->_width*oldPicSize->_height) ) return;

    const float x     = (float(idx%newPicSize->_width) + 0.5) * widthRatio - 0.5;
    const int sumFrom = int(x) - a + 1;
    const int sumTo   = int(x) + a;
    
    CPixel sum  = {};
    for( int kernelIdx = sumFrom; kernelIdx <= sumTo; ++kernelIdx )
    {
        sum += oldPicture[clamp( kernelIdx, oldPicSize->_width - 1 ) + (idx/newPicSize->_width)*oldPicSize->_width] * lanczosKernel(x - kernelIdx);
    }

    sum.clampRGB();
    interPicture[idx] = sum;
}

// Lanczos Resampling V function -- resampling vertically
__global__ void lanczosResamplingV( const CPixel* interPicture, CPixel* newPicture, const SSize* oldPicSize, const SSize* newPicSize )
{
    const float heightRatio = float(oldPicSize->_height) / float(newPicSize->_height);
    const int idx           = threadIdx.x + (blockIdx.x * blockDim.x);
    if( idx >= newPicSize->total() ) return;

    const float x     = (float(idx/newPicSize->_width) + 0.5) * heightRatio - 0.5;
    const int sumFrom = int(x) - a + 1;
    const int sumTo   = int(x) + a;
    ...
    for( int kernelIdx = sumFrom; kernelIdx <= sumTo; ++kernelIdx )
    {
        sum += interPicture[( clamp( kernelIdx, oldPicSize->_height - 1 ) * newPicSize->_width ) + idx%newPicSize->_width] * lanczosKernel(x - kernelIdx);
    }
...
int main( int argc, char** argv )
{
    ...
    CPixel* gpuInterPicture;
    cudaMalloc( (void**)&gpuInterPicture, newPicSize._width*oldPicSize._height*sizeof( CPixel ) );
    ERROR_HANDLER( cudaGetLastError() );
    ...
    size_t InterBlockCount = ((newPicSize._width*oldPicSize._height) / THREADS_PER_BLOCK_LIMIT) + 1;
    size_t blockCount      = (newPicSize.total() / THREADS_PER_BLOCK_LIMIT) + 1;
    auto startTimer = std::chrono::high_resolution_clock::now();

    lanczosResamplingH<<<InterBlockCount, THREADS_PER_BLOCK_LIMIT>>>( gpuOldPicture, gpuInterPicture, gpuOldPicSize, gpuNewPicSize );
    cudaDeviceSynchronize();
    ERROR_HANDLER( cudaGetLastError() );

    lanczosResamplingV<<<blockCount, THREADS_PER_BLOCK_LIMIT>>>( gpuInterPicture, gpuNewPicture, gpuOldPicSize, gpuNewPicSize );
    cudaDeviceSynchronize();
    ERROR_HANDLER( cudaGetLastError() );
